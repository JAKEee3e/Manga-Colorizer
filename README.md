...
BACKEND_DIR = "/teamspace/studios/this_studio/Manga-Colorizer/Backend"
...
def run_backend():
    ...
    print(
        f"[!] Backend process exited with code {code}. Last logs:\n"
...
while waited < max_wait:
    if python_process is not None:
        ...
        if code is not None:
            tail = "\n".join(list(backend_log_tail)[-60:])
            raise RuntimeError(
                f"Backend process exited early with code {code}. Last logs:\n{tail}"
            )
    ...
    waited += 3
    if waited % 15 == 0:
        print(f"[*] Still waiting for backend... ({waited}s/{max_wait}s)")
...
if public_url:
    ...
    print(f"[*] Run manually: {cloudflared_path} tunnel --url http://127.0.0.1:{PORT}")
